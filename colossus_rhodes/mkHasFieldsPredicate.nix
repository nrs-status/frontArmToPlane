...
let total = rec {
  ...
  predicate = rec {
    predname = "hasFields";
    function = target: { testresult = length (attrNames (failures target)) > 0; inherit failures; };
    handler = { target, type }: 
      if (function target).testresult then
        throw ("${type.typeName} failed to typecheck due to hasFields predicate; the following fields are unexpected: ${attrNames result.failures}")
      else target;
  };
  final = predicate;
  };
